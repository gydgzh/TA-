...
Shader "Unlit/KKK/UnlitShader_K03_My"//shader的名字
{
    Properties
    //对应材质面板控制的参数
    {
    ...
    SubShader
    {
        Tags {"Queue" = "Transparent"}//不透明物体记得改渲染队列
        Pass//在Pass里面写shader代码、一个shader里也可以有多个PASS
        {
            ...
            v2f vert(appdata v)//定点着色器
            {
                ...
                //Tiling X：控制贴图在 U 方向（横向） 重复多少次
                //Tiling Y：控制贴图在 V 方向（纵向） 重复多少次
                //Offset X：贴图沿 U（横向） 滚动/平移
                //Offset Y：贴图沿 V（纵向） 滚动/平移
                o.pos_uv = v.vertex.xy  * _MainTexture_ST.xy + _MainTexture_ST.zw;
                //用模型空间上的顶点，就能平整地对应过去
            ...
            {
                float4 col = tex2D(_MainTexture,i.uv) * _MainColor;
